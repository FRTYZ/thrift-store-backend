...
exports.putAdvertEdit =  async function (req: Request, res: Response, next: NextFunction) {
    ...
    const {title, description, how_status, price, city_id, county_id, old_images} = req.body;

    const advertImages = req.files;
    const deletedImages = JSON.parse(req.body.old_images);

    try {
        if (!advert_id || advert_id == '') {
            throw new CustomError(403, "advert_id alanını belirtmelisiniz.");
        }
        if (!title || title == '') {
        ...
        const selectQuery = `
            ...
            AND
                ad.user_id = $2
        `
        const selectResponse = await pool.query(selectQuery, [advert_id, user_id]);
        ...
        const updateQuery = `
            ...
            WHERE
                id = $7
        `;
        const updateValues = [title, description, price, city_id, county_id, how_status, advert_id]
